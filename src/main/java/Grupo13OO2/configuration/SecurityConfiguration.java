package Grupo13OO2.configuration;
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests()
				.antMatchers("/css/*", "/imgs/*", "/js/*", "/vendor/bootstrap/css/*", "/vendor/jquery/*",
						"/vendor/bootstrap/js/*").permitAll().antMatchers("/").permitAll().antMatchers("/manager/**").hasRole("GERENTE").anyRequest().authenticated().and().formLogin().loginPage("/login")
