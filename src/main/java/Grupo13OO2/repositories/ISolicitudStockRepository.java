...
public interface ISolicitudStockRepository extends JpaRepository<SolicitudStock, Integer> {
	...
	@Query(value = "SELECT pe.*,s.* FROM solicitud_stock s INNER JOIN pedido pe on pe.id= s.id WHERE s.local_id = ?1 ORDER BY ?#{#pageable}",
       countQuery = "SELECT count(*) FROM solicitud_stock WHERE local_id = ?1",
       nativeQuery = true)
